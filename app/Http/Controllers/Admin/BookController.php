    ...
    public function finishPrint(Request $request)
    {
        ...
        foreach($request->book as $book){
            if(isset($book['selected_subjects'])){
                if($book['selected_subjects'] == 1){
                    $books = Book::find($book['id']);
                    $bookTarget = TargetBook::where('book_id', $book['id'])->first();
                    if ($book['print_quantity'] > $bookTarget->target - $books->quantity) {
                        toastr()->error('قيمة كمية الطباعة يجب أن تكون أقل من أو تساوي ' . $bookTarget->target  - $books->quantity);
                        return back();
                    }
                    $books->quantity += $book['print_quantity'];
                    $books->save();
                    $count++;
                }
            }
